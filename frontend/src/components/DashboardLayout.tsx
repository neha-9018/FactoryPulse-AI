...
export default function DashboardLayout() {
  ...
  const simulateOfflineResponse = (query: string) => {
    ...
    if (queryLower.includes("maintenance") || queryLower.includes("risk")) {
      ...
      responseText += "I am online and querying your database. However, I didn't recognize that specific question. Ask about *'maintenance'*, *'production yields'*, or *'defect trends'* to test SQL retrieval!";
    }
  ...
  const renderMessageText = (text: string) => {
    return text.split("\n").map((line, i) => {
      ...
      if (line.startsWith("|")) {
        ...
        return (
          <div key={i} className="grid grid-cols-3 gap-2 py-1 border-b border-brand-border/40 text-xs font-mono">
            {cols.map((col, idx) => (
  ...
  const navItems = [
    ...
    { name: "Intelligent Logistics", path: "/logistics", icon: Truck, roles: ["ADMIN", "MANAGER", "ENGINEER", "OPERATOR"] },
  ...
  return (
    ...
    {/* 6. Sliding AI Assistant Drawer */}
      <div 
        className={`fixed top-0 right-0 h-full w-full sm:w-[450px] bg-brand-card/95 border-l border-brand-border backdrop-blur-md shadow-2xl z-50 flex flex-col transition-all duration-300 transform ${
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <span className="text-[10px] text-emerald-400 font-semibold flex items-center gap-1">
                ...
                Gemini RAG Connected
              </span>
            </div>
          </div>
          <button 
            onClick={() => setAssistantOpen(false)}
            className="p-2 text-slate-400 hover:text-white hover:bg-brand-border rounded-lg transition"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {/* Messages List */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-brand-bg/35 scrollbar-thin text-left">
          {messages.map((m, index) => (
            <div 
              key={index}
              className={`flex ${m.sender === "user" ? "justify-end" : "justify-start"}`}
            >
              <div className={`max-w-[85%] px-4 py-3 rounded-2xl border text-sm ${
                m.sender === "user" 
          ...
          {assistantLoading && (
            <div className="flex justify-start">
              <div className="max-w-[80%] px-4 py-3 rounded-2xl border border-brand-border bg-brand-card text-slate-400 rounded-bl-none text-xs flex items-center gap-2">
        ...
        {/* Quick Prompts Panel */}
        <div className="px-6 py-3 border-t border-brand-border/60 bg-brand-card/45 flex flex-wrap gap-2">
          {quickPrompts.map((p, idx) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 bg-brand-bg hover:bg-brand-border border border-brand-border rounded-xl text-[10px] text-slate-300 hover:text-white font-medium transition duration-150"
              >
      ...
      {!assistantOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 p-4 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white rounded-full shadow-glow z-40 flex items-center justify-center transition-all duration-200 transform hover:scale-105 animate-bounce"
